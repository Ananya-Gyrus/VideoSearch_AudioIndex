...
- Saves metadata + embedding summary to a CSV file instead of a SQLite DB
"""
...
try:
    ...
    print("ERROR importing project utilities.", e)
    raise

def process_video_file(path, extractor, model, device, num_frames=8):
    meta = {'filename': os.path.basename(path), 'filepath': os.path.abspath(path)}
    try:
